({
    getOpporunityStage: function (cmp, event) {
        var action = cmp.get('c.getOpportunityStages');
        var options = [];
        action.setCallback(this, function (response) {
            if (response.getState() == 'SUCCESS') {
                ...
                for (let i = 0; i < result.length; i++) {
                    options.push({ 'Id': result[i], 'Name': result[i] });
                }
    ...
    navigateClick: function (cmp, event) {
        const navService = cmp.find('navService');
        var selectedStages = cmp.get("v.selectedOptions");
        var stages = [];

        selectedStages.forEach((option, index) => {
        ...
        var pageReference = {
            ...
            attributes: {
                componentName: 'c__OpportunityList'
            },
            state: {
                "c__stages": stages
            }
